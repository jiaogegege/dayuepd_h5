  ...
  export default {
    ...
    methods: {
      ...
      getVerCode () {
        ...
        this.$http.registerGetVerCode(this.userPhone)
            .then(res => {
              loading.close();
      ...
      getSpeechCode () {
        const loading = this.$loading();
        this.$http.registerGetSpeechCode(this.userPhone)
            .then(res => {
              loading.close();
              this.$toast.success('发送成功');
              this.$refs.countdownBtn.countDown(60);
      ...
      nextAction () {
        const loading = this.$loading();
        this.$http.registerCheckCode(this.userPhone, this.code)
            .then(res => {
              loading.close();
              //跳转设置密码界面
              this.$router.push({name: 'PDRegisterSetPsdPage'});
            })
            .catch(err => {
              loading.close();
              this.$toast.error(err.message);
