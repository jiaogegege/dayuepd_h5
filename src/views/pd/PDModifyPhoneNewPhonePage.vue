          ...
          <mu-list class="list">
            ...
            <mu-flex class="list-item" direction="column" align-items="center">
              <mu-flex class="list-item-top" justify-content="start" align-items="center">
                <div style="margin-right: 35px;">新手机号</div>
                <mu-text-field class="new-phone text-field"
                               color="primary"
                               solo
                               v-model="newPhone"
                               style="width: 200px;"
...
<script>
  ...
  export default {
    ...
    computed: {
      ...
      btnActive () {
        return this.verCode.length > 0 && this.newPhone.length > 0;
      },
    ...
    methods: {
      ...
      //倒计时按钮开始倒计时的事件
      getVerCode() {
        if (!this.$util.checkPhoneFormat(this.newPhone.trim())) {
          this.$toast.error(this.$str.phoneFormatErrorHint);
          return;
        }
        //请求验证码接口
        const loading = this.$loading();
        this.$http.modifyPhoneSendNewVerCode(this.getUserId, this.newPhone.trim())
            .then(res => {
              loading.close();
      ...
      getSpeechCode() {
        const loading = this.$loading();
        this.$http.modifyPhoneSendNewSpeechCode(this.getUserId, this.newPhone.trim())
            .then(res => {
              loading.close();
              this.$toast.success('发送成功');
              this.$refs.countdownBtn.countDown(60);
      ...
      submitAction() {
        const loading = this.$loading();
        this.$http.modifyPhoneNewPhone(this.getUserId, this.newPhone.trim(), this.verCode)
            .then(res => {
              loading.close();
              this.setModifyUserPhone(this.newPhone.trim());
              //返回系统设置界面
              this.$router.push({name: 'PDSystemSettingPage'});
            })
            .catch(err => {
              loading.close();
              this.$toast.error(err.message);
